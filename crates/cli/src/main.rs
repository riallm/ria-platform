...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Serve { model, tokenizer, host, port, device: device_str, max_seq_len } => {
            cmd_serve(&model, tokenizer.as_ref(), &host, port, &device_str, max_seq_len).await?;
        }
        Commands::Generate { model, tokenizer, prompt, max_tokens, temperature, top_p, device } => {
            cmd_generate(&model, tokenizer.as_ref(), &prompt, max_tokens, temperature, top_p, &device).await?;
        }
...
async fn cmd_serve(
    ...
    use ria_core::{RIAModel, RIATokenizer};
...
async fn cmd_generate(
    ...
    use ria_core::{RIAModel, RIATokenizer, Generator, GenerationConfig};
    use candle_core::Device;
