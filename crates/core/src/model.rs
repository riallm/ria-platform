...
use candle_nn::{RmsNorm, Linear, Embedding};
use ria_gguf::{GGUFReader, GGUFQuantizationType};
...
impl RIAModel {
    ...
    pub fn from_gguf(
        ...
        let tensors = reader.load_all_tensors(&device)
            .map_err(RIAError::GGUF)?;
    ...
    fn parse_config(reader: &GGUFReader) -> Result<ModelConfig> {
        ...
        let get_u32 = |key: &str| -> Result<u32> {
            match reader.get_metadata(key) {
                ...
                _ => Err(RIAError::ModelLoading(format!("Missing/invalid metadata: {}", key))),
            }
        };
        ...
        let context_length = get_u32("llama.context_length")
            .or_else(|_| get_u32("ria.context_length"))?;
        let embedding_length = get_u32("llama.embedding_length")
            .or_else(|_| get_u32("ria.embedding_length"))?;
        let block_count = get_u32("llama.block_count")
            .or_else(|_| get_u32("ria.block_count"))?;
        let feed_forward_length = get_u32("llama.feed_forward_length")
            .or_else(|_| get_u32("ria.feed_forward_length"))?;
        ...
        let vocab_size = get_u32("llama.vocab_size")
            .or_else(|_| get_u32("ria.vocab_size"))?;
    ...
    fn build_from_tensors(
        ...
        for i in 0..config.block_count {
            ...
            let layer = TransformerBlock::from_tensors(
                &prefix, config, tensors, device.clone(), i as usize
            )?;
            layers.push(layer);
        }
        ...
        let norm = RmsNorm::new(
            norm_weight,
            config.layer_norm_rms_epsilon as f64,
        );
    ...
    pub fn forward(
        ...
        let mut hidden = self.tok_embeddings.embeddings().index_select(input_ids, 0)?;
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            let attn_output = attn_output.transpose(1, 2)?
                .reshape(((), seq_len, self.config.embedding_length as usize))?;
...
fn apply_rope(
    ...
    let positions: Vec<u32> = (seq_offset..(seq_offset + seq_len)).map(|i| i as u32).collect();
...
fn rope_apply_interleaved(x: &Tensor, cos: &Tensor, sin: &Tensor) -> Result<Tensor> {
    ...
    let out_first = ((x_first_half.clone() * cos_bc.clone())? - (x_second_half.clone() * sin_bc.clone())?)?;
    let out_second = ((x_second_half * cos_bc)? + (x_first_half * sin_bc)?)?;
    
    // Concatenate: [out_first, out_second] -> [batch, heads, seq, head_dim]
    Ok(Tensor::cat(&[&out_first, &out_second], candle_core::D::Minus1)?)
}
